    ...
    ISpectreAllocations
{
    ...
    function setSpectreAddress(address _spectre) external onlyOwner {
        require(
            _spectre != address(0),
            "setSpectreAddress: address can't be zero address"
        );
        require(
            _spectre != address(spectreContract),
            "setSpectreAddress: provided address must differ from existing one"
        );
        spectreContract = IERC721Upgradeable(_spectre);
    }

    function setTetherAddress(address _tether) external onlyOwner {
        require(
            _tether != address(0),
            "setTetherAddress: address can't be zero address"
        );
        require(
            _tether != tetherContract,
            "setTetherAddress: provided address must differ from existing one"
        );
        tetherContract = _tether;
    }

    function setUSDC(address _USDC) external onlyOwner {
        require(_USDC != address(0), "setUSDC: address can't be zero address");
        require(
            _USDC != address(USDC),
            "setUSDC: provided address must differ from existing one"
        );
        USDC = IERC20(_USDC);
    }

    function setUSDT(address _USDT) external onlyOwner {
        require(_USDT != address(0), "setUSDT: address can't be zero address");
        require(
            _USDT != address(USDT),
            "setUSDT: provided address must differ from existing one"
        );
        USDT = IERC20(_USDT);
    }
